...
async function sendReaction(client, msg, emoji = "👀") {
    ...
    let transactionId = "__botzilla_confession_set_seen_" + reactionId++;
...
async function handler(client, msg, extra) {
    ...
    if (!roomAlias && extra.homeserverDomain !== null) {
        let stateKey = extra.homeserverDomain;
        let resp = await client.getRoomStateEvent(msg.room, 'm.room.aliases', stateKey);
        for (let alias of resp.aliases) {
    ...
    let repo = GITHUB_CLIENT.repo('mrgiggles/histoire');
